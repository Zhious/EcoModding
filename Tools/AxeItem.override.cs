    ...
    public abstract partial class AxeItem : MeleeWeaponItem, IInteractor
    {
        ...
        public bool Chop(Player player, InteractionTriggerInfo triggerInfo, InteractionTarget target)
        {
            if (triggerInfo == InteractionTrigger.LeftClick)
            {
                ...
                {
                    // Check if player is a Logger speciality. If so, do nothing to damage. If not, reduce damage by 20%. (At .834f, if a tree takes 10 hits to normally destroy, it will take 11.99 hits (~12, 20% more).
                    var DamageCalc = 1f;
                    if ((player.User.Skillset.GetSkill(typeof(LoggingSkill))).Level <= 0) { DamageCalc = .834f; }

                    var action = tree.TryApplyDamage(new(), player, (this.Damage.GetCurrentValue(player!.User) + this.PerkDamage.GetCurrentValue(player!.User)) * DamageCalc, target, this, out _);
